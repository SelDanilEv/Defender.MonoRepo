...
public sealed class MaintenanceValidationTests
{
    ...
    public void Update_ManualBaselineLockedWhenHistoryLinkedButNameAndIntervalRemainEditable()
    {
        ...
        var exception = Assert.Throws<CarDomainException>(() => item.SetManualBaseline(new DateOnly(2026, 1, 1), 10_000, true, Clock, false));
    ...
    public void Update_RejectsArchivedVehicleContext()
    {
        ...
        var item = MaintenanceItem.Create(vehicle, "Oil", 12, null, null, null, Clock);
        vehicle.Archive(Clock);

        var exception = Assert.Throws<CarDomainException>(() => item.Update(vehicle, "Brake oil", 6, null, Clock));

        Assert.Equal("CAR_VEHICLE_ARCHIVED", exception.Code);
    }

    [Fact]
    public void Update_InvalidIntervalLeavesPriorStateUnchanged()
    {
        var item = MaintenanceItem.Create(Guid.NewGuid(), Guid.NewGuid(), "Oil", 12, 2, null, null, Clock);

        Assert.Throws<CarDomainException>(() => item.Update("Brake oil", 0, null, false, Clock));

        Assert.Equal("Oil", item.Name);
        Assert.Equal(12, item.IntervalMonths);
        Assert.Equal(2, item.IntervalThousandKm);
    }
}
