...
Definitions use integer minor-unit amounts. `Regular` and `Subscription` amounts contribute their full monthly amount; `Annual` contributes `Amount / 12m` without premature rounding. Review snapshots retain definition metadata and captured exchange rates, so later definition edits do not rewrite history. A review template without an explicit month uses the current normalized month and fetches rates from the previous UTC calendar day; an explicit month fetches rates from that month's first day.
