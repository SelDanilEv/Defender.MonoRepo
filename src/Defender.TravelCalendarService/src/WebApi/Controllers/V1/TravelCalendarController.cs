...
public class TravelCalendarController(ICurrentAccountAccessor account, ITravelCalendarService service) : ControllerBase
{
    ...
    [HttpPost("events")] public async Task<ActionResult<TravelCalendarMutationResultDto>> CreateEvent(CreateTravelEventRequest request, CancellationToken ct) => StatusCode(201, await service.CreateEventAsync(UserId, request, ct));
