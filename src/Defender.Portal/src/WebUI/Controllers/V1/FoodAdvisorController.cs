...
using Defender.Portal.Application.Models.ApiRequests.FoodAdvisor;
using Defender.Portal.Application.Modules.FoodAdvisor.Commands;
using Defender.Portal.Application.Modules.FoodAdvisor.Queries;
using AutoMapper;
using MediatR;
...
public class FoodAdvisorController(
    IMediator mediator,
    IMapper mapper,
    IPersonalFoodAdvisorWrapper foodAdvisorWrapper) : BaseApiController(mediator, mapper)
{
    [HttpGet("preferences")]
    [Auth(Roles.User)]
    ...
    public async Task<ActionResult> UploadSessionImages(Guid sessionId, [FromForm] IFormFileCollection? files, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var refs = await foodAdvisorWrapper.UploadSessionImagesAsync(sessionId, streams.ToArray(), contentTypes.ToArray(), cancellationToken);
            return Ok(refs);
        }
        finally
        {
