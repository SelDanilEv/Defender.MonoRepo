...
{
    [HttpGet, Auth(Roles.User)] public async Task<IActionResult> Get([FromQuery] string? from, [FromQuery] string? to, CancellationToken ct) => Ok(await wrapper.GetAsync(from, to, ct));
