...
export const EventDrawer = ({
  ...
  return <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: 480, maxWidth: "100vw", p: { xs: 2, sm: 3 }, background: "var(--tc-drawer)", color: "var(--tc-text)", backgroundImage: "none" } }}>
    <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
      <Box>
        <Typography component="h2" variant="h5" fontWeight={900}>{t("travelCalendar:drawer.title")}</Typography>
        <Typography color="var(--tc-muted)" fontSize={13}>
    ...
    <Stack spacing={2}>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}>
        <TextField label={t("travelCalendar:drawer.fields.startDate")} type="date" value={draft.startDate} disabled={!event.canEdit} onChange={(e) => set("startDate", e.target.value)} InputLabelProps={{ shrink: true }} fullWidth />
        <TextField label={t("travelCalendar:drawer.fields.endDate")} type="date" value={draft.endDate} disabled={!event.canEdit} onChange={(e) => set("endDate", e.target.value)} InputLabelProps={{ shrink: true }} fullWidth />
      </Stack>
      ...
      <Box sx={{ p: 2, border: "1px solid var(--tc-border)", borderRadius: 3 }}>
        <Stack direction="row" alignItems="center" spacing={1} mb={1}>
          ...
          <Typography fontWeight={800}>{t("travelCalendar:drawer.participants")}</Typography>
        </Stack>
        <Stack spacing={1}>
          {event.participants.map((participant) => <Stack key={participant.userId} direction="row" spacing={1} alignItems="center">
            <Chip size="small" label={participant.displayName} color={participantColor(participant.status)} />
            <Typography fontSize={12} color="var(--tc-muted)">{t(`travelCalendar:statuses.${participant.status}`)}</Typography>
      ...
      {overnight && <Box sx={{ p: 2, border: "1px solid var(--tc-border)", borderRadius: 3 }}>
        <Typography fontWeight={800}>🏨 {t("travelCalendar:drawer.hotel")}</Typography>
      ...
      {isTrip && <Box sx={{ p: 2, border: "1px solid var(--tc-border)", borderRadius: 3 }}>
        <Typography fontWeight={800} mb={1}>📍 {t("travelCalendar:drawer.points")}</Typography>
        ...
        {event.points.map((item) => <Stack key={item.id} direction="row" alignItems="center">
        ...
        {event.canEdit && <Stack direction="row" mt={1}>
      ...
      <Box sx={{ p: 2, borderRadius: 3, bgcolor: "var(--tc-accent-soft)" }}>
        <Typography fontWeight={900} mb={1}>{t("travelCalendar:drawer.eventBudget")}</Typography>
        <Stack spacing={1.5}>
        ...
        <Stack direction="row" justifyContent="space-between" mt={2}><Typography fontWeight={800}>{t("travelCalendar:drawer.total")}</Typography><Typography fontWeight={900} color="var(--tc-accent)">{formatMoney(total)} PLN</Typography></Stack>
      </Box>
