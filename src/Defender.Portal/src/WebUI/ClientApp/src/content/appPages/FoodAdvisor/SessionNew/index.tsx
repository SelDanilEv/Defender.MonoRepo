...
const FoodAdvisorSessionNewPage = () => {
  const u = useUtils();
  ...
  const pollSession = useCallback(
    (sessionId: string) => {
      return foodAdvisorApi.getSession(sessionId, u).then((data) => {
        if (!data) return;
        ...
        if (data.status === STATUS_REVIEW) {
          ...
          setParsing(false);
          navigate(`/food-advisor/session/${sessionId}/review`, {
            replace: true,
          });
  ...
  const applySelectedFiles = useCallback((inputFiles: File[]) => {
    ...
    if (imageFiles.length !== inputFiles.length) {
      u?.e?.(u.t("foodAdvisor:session_only_images_error"));
    }
    ...
    if (imageFiles.length > MAX_IMAGES) {
      u?.e?.(u.t("foodAdvisor:session_max_images_error"));
  ...
  const handleUploadAndParse = async () => {
    ...
    try {
      ...
      if (!activeSession?.id) {
        ...
        activeSession = await foodAdvisorApi.createSession(u);
        setSession(activeSession);
      }

      await foodAdvisorApi.uploadSessionImages(activeSession.id, files, u);
      await foodAdvisorApi.requestParsing(activeSession.id, u);
      setParsing(true);
    } catch {
      setParsing(false);
    } finally {
            ...
            <CardContent>
                ...
                >
                  <Stack spacing={1.5} alignItems={{ xs: "stretch", sm: "flex-start" }}>
                    ...
                    >
                      ...
                      >
                        {files.length > 0
                          ? u.t("foodAdvisor:session_replace_images")
                          : u.t("foodAdvisor:session_select_images")}
                        <input
                          hidden
                          type="file"
                          accept="image/*"
                        ...
                        <ListItem
                          ...
                          secondaryAction={(
                            <IconButton
                              ...
                              size="small"
                              color="error"
                              aria-label={u.t("foodAdvisor:session_remove_image")}
              ...
              {pollTimedOut && (
                <Alert severity="warning" sx={{ mt: 1.5 }}>
                  {u.t("foodAdvisor:session_parsing_timeout_warning")}
                </Alert>
              )}
              {session?.status === STATUS_FAILED && (
                <Alert severity="error" sx={{ mt: 1.5 }}>
                  {u.t("foodAdvisor:session_parsing_failed_warning")}
                </Alert>
              )}
            </CardContent>
          </Card>
...
export default connect(mapStateToProps)(FoodAdvisorSessionNewPage);
