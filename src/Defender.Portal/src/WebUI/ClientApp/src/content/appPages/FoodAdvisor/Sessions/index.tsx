...
const getSessionActionPath = (session: MenuSessionDto) => {
  ...
  if (status === "confirmed" || session.rankedItems.length > 0) {
    return `/food-advisor/session/${session.id}/recommendations`;
  }

  return `/food-advisor/session/${session.id}/review`;
};
...
export const FoodAdvisorSessionsPage = () => {
  const u = useUtils();
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      foodAdvisorApi.getSessions(utilsRef.current),
      foodAdvisorApi.getRatings(utilsRef.current),
    ])
    ...
    <Box sx={{ pt: 1 }}>
      <Box sx={{ mb: 2.5, textAlign: "center" }}>
        <Typography variant="h4" sx={{ mb: 1 }}>
          {u.t("foodAdvisor:sessions_title")}
        </Typography>
        <Typography
          variant="body2"
      ...
      <Grid container spacing={2}>
        ...
        {pagedSessions.map((session) => {
          ...
          const sessionStatusKey = `foodAdvisor:status_${session.status.toLowerCase()}`;
                ...
                <CardContent>
                  ...
                  >
                    ...
                    <Box display="flex" gap={1} flexWrap="wrap" alignItems="flex-start">
                      ...
                      >
                        {detailsExpanded
                          ? u.t("foodAdvisor:sessions_hide_dishes")
                          : u.t("foodAdvisor:sessions_show_dishes")}
                      </Button>
                      <Button
                        variant="outlined"
                        color="error"
                      ...
                      >
                        {deletingSessionId === session.id
                          ? u.t("foodAdvisor:polling")
                          : u.t("foodAdvisor:sessions_delete")}
                      </Button>
                    </Box>
                  </Stack>
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                      ...
                      <Typography variant="body2">
                        {session.trySomethingNew
                          ? u.t("foodAdvisor:sessions_yes")
                          : u.t("foodAdvisor:sessions_no")}
                      </Typography>
                    </Grid>
                  </Grid>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <Box display="flex" gap={1} alignItems="center" flexWrap="wrap">
                                    ...
                                    >
                                      {rating
                                        ? u.t("foodAdvisor:sessions_edit_rating")
                                        : u.t("foodAdvisor:sessions_rate")}
                                    </Button>
                                  </Box>
                                </Box>
                              );
          ...
          <Box display="flex" gap={1} justifyContent="flex-end">
            ...
            >
              {deletingSessionId && deletingSessionId === deleteTarget?.id
                ? u.t("foodAdvisor:polling")
                : u.t("foodAdvisor:sessions_delete_submit")}
            </Button>
          </Box>
        </Stack>
      </CustomDialog>
...
export default connect(mapStateToProps)(FoodAdvisorSessionsPage);
