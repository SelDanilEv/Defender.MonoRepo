...
import { getVehicle, getHistory, createMaintenanceItem, updateMaintenanceItem, deleteMaintenanceItem } from "src/api/myGarage";
...
import type { MaintenanceItem, VehicleDetail, ServiceHistoryPage } from "src/models/myGarage/CarModels";
...
export default function MaintenancePage() {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const [nextDetail, nextHistory] = await Promise.all([
        ...
        getHistory(vehicleId, 0, 100, null, signal),
      ]);
  ...
  const linkedIds = useMemo(() => new Set((history?.items ?? []).flatMap((record) => record.linkedMaintenanceItemIds)), [history]);
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3, lg: 4 } }}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ alignItems: { sm: "center" }, mb: 3 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate(`/my-garage/vehicles/${vehicle.id}`)}>{vehicle.displayName}</Button>
      ...
      <Card><CardContent sx={{ p: 0, "&:last-child": { pb: 0 } }}>
        <GarageTable ariaLabel={t("actions.addMaintenance")} headers={[t("fields.name"), t("fields.intervalMonths"), t("fields.intervalThousandKm"), !u.isMobile ? t("fields.lastDate") : null, !u.isMobile ? t("fields.nextDate") : null, t("statuses.Upcoming"), t("table_actions_column", { defaultValue: "Actions" })]} empty={maintenanceItems.length === 0} emptyMessage={t("empty.maintenance")}>
          {maintenanceItems.map((item) => {
            const referenced = linkedIds.has(item.id);
            return <TableRow hover key={item.id}>
              ...
              {!u.isMobile ? <TableCell>{formatDate(item.lastDate, locale)}</TableCell> : null}
              {!u.isMobile ? <TableCell>{formatDate(item.nextDate, locale)}{item.nextOdometerKm === null ? "" : ` · ${item.nextOdometerKm.toLocaleString(locale)} ${t("units.km")}`}</TableCell> : null}
      ...
      <MaintenanceDialog open={dialogOpen} item={selectedItem} baselineLocked={Boolean(selectedItem && linkedIds.has(selectedItem.id))} busy={mutating} submitError={submitError} onClose={() => setDialogOpen(false)} onSubmit={submit} />
