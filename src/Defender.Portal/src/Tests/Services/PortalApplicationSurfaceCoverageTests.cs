...
public class PortalApplicationSurfaceCoverageTests
{
    [Fact]
    public async Task PublicTypes_WhenConstructedAndSafeMethodsInvoked_IncreaseCoverage()
    {
        var assembly = typeof(Defender.Portal.Application.Modules.Authorization.Commands.LoginWithPasswordCommand).Assembly;
        var types = assembly.GetTypes()
            .Where(t => t.IsClass && !t.IsAbstract)
            .Where(t => t.Namespace is not null && t.Namespace.StartsWith("Defender.Portal.Application", StringComparison.Ordinal))
