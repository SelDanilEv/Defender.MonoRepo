...
public class RiskGamesApplicationSurfaceCoverageTests
{
    [Fact]
    public async Task PublicTypes_WhenConstructedAndSafeMethodsInvoked_IncreaseCoverage()
    {
        var assembly = typeof(Defender.RiskGamesService.Application.Modules.Lottery.Commands.CreateLotteryCommand).Assembly;
        var types = assembly.GetTypes()
            .Where(t => t.IsClass && !t.IsAbstract)
            .Where(t => t.Namespace is not null && t.Namespace.StartsWith("Defender.RiskGamesService.Application", StringComparison.Ordinal))
