...
public class NotificationApplicationSurfaceCoverageTests
{
    [Fact]
    public async Task PublicTypes_WhenConstructedAndSafeMethodsInvoked_IncreaseCoverage()
    {
        var assembly = typeof(Defender.NotificationService.Application.Modules.Notifications.Commands.SendPureEmailCommand).Assembly;
        var types = assembly.GetTypes()
            .Where(t => t.IsClass && !t.IsAbstract)
            .Where(t => t.Namespace is not null && t.Namespace.StartsWith("Defender.NotificationService.Application", StringComparison.Ordinal))
