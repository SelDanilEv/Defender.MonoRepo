...
public class PersonalFoodAdvisorApplicationSurfaceCoverageTests
{
    [Fact]
    public async Task PublicTypes_WhenConstructedAndSafeMethodsInvoked_IncreaseCoverage()
    {
        var assembly = typeof(Defender.PersonalFoodAdvisor.Application.Modules.Module.Commands.ModuleCommand).Assembly;
        var types = assembly.GetTypes()
            .Where(t => t.IsClass && !t.IsAbstract)
            .Where(t => t.Namespace is not null && t.Namespace.StartsWith("Defender.PersonalFoodAdvisor.Application", StringComparison.Ordinal))
