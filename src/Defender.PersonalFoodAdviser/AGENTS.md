# AGENTS Guide: Defender.PersonalFoodAdviser

## Purpose
- Handles Personal Food Adviser workflows: user preferences, menu image parsing, menu confirmation, dish ratings, and recommendation generation.

## Project entry points
- Solution: `Defender.PersonalFoodAdviser.sln`
...
- Kafka + outbox background workers: `src/Application/Services/Background/Kafka/`
  - `FoodAdviserEventListenerService`, `MenuParsingOutboxPublisherService`, `RecommendationsOutboxPublisherService`, `CreateKafkaTopicsService`
...
- BFF endpoints live in `src/Defender.Portal/src/WebUI/Controllers/V1/FoodAdviserController.cs`.
- Client pages live in `src/Defender.Portal/src/WebUI/ClientApp/src/content/appPages/FoodAdviser/`.
- For Food Adviser UI changes in React, use Material UI (`@mui/material`, `@mui/icons-material`) as the default component library.
...
- Parsing reliability changes:
  ...
  - Then `FoodAdviserEventListenerService` + `MenuParsingProcessor` (consume/process).
...
- Build: `dotnet build Defender.PersonalFoodAdviser.sln`
- Test: `dotnet test Defender.PersonalFoodAdviser.sln`
- Test project only: `dotnet test src/Tests/Defender.PersonalFoodAdviser.Tests.csproj`
